...
Idiomatic Python bindings for the Polyglot native structured logger.
...
Set `POLYGLOT_LOGGER_LIB` to the absolute path of `logger.dll`, `liblogger.so`, or `liblogger.dylib` if the library is not found automatically.
