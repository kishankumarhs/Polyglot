# Docs

Shared logging core (Go) + C ABI + bindings for Python, Node, and .NET.

## Start here

- [SDK comparison](sdk.md) — Node / Python / .NET side by side
- [First log](first-log.md) — install a package, print a line
- [Getting started](getting-started.md) — build from source
- [User guide](user-guide.md) — levels, fields, async, flush
- [Zero-config](zero-config.md) — `polyglot.yaml` discovery + merge
- [Configuration](configuration.md)
- [Sinks](sinks.md)
- [Benchmarks](../bench/README.md)
- [Compatibility](compatibility.md)
- [ABI mismatch policy](compatibility-policy.md)
- [v1.0 checklist](../V1-CHECKLIST.md)

## Migrations

- [Pino](migrate-from-pino.md)
- [Zap](migrate-from-zap.md)
- [Serilog](migrate-from-serilog.md)
- [Python logging / Structlog](migrate-from-python-logging.md)

## By language

- [Python](languages/python.md)
- [Node.js / TypeScript](languages/node.md)
- [.NET](languages/dotnet.md)
- [Go (direct)](languages/go.md)
- [C / other FFI](languages/c.md)

## Maintainers

- [Build & packaging](build.md)
- [Monorepo](monorepo.md)
- [Architecture](architecture.md)
- [ABI & codegen](abi.md)
- [Troubleshooting](troubleshooting.md)

Root overview: [../README.md](../README.md) · Examples: [../examples/](../examples/) · ABI: [../api/abi.json](../api/abi.json)
